...
\newcommand{\Fill}[1]{%
    \makebox[6cm][c]{#1}\hspace{-6cm}\rule[-0.5pt]{6cm}{0.4pt}
}
...
\begin{tabular}{rl}
    课程设计名称： & \Fill{数据结构课程设计} \\
    专\qquad\quad 业： & \Fill{计算机科学与技术} \\
    学\qquad\quad 号： & \Fill{2510801122} \\
    学 生 姓 名： & \Fill{吴亚洲} \\
    成\qquad\quad 绩： & \Fill{} \\
    批改日期： & \Fill{} \\
    指导教师： & \Fill{} \\
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{一元多项式计算流程图}
        \label{fig:polynomial_flowchart1}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        ...
        \caption{一元多项式运算详细流程图}
        \label{fig:polynomial_flowchart2}
    \end{minipage}
...
\subsection {2.哈夫曼编码}
\subsubsection {任务}
要求对输入的一串文本字符实现哈夫曼编码，再对哈夫曼编码生成的 0、1 串进行译码，输出原始文本字符串。
功能要求：
（1）统计文本字符串中不同字符的个数以及各字符在文本中出现的次数；
（2）根据字符数量与对应频度建立并打印哈夫曼树；
（3）生成并输出每个字符对应的哈夫曼前缀编码；
（4）将原始文本转换为连续 01 二进制编码序列；
（5）输入 01 编码串，基于哈夫曼树译码还原原始文本。
界面要求：程序运行后，输出功能菜单并提示输入选项：
统计字符的个数以及字符出现的次数
建立并输出哈夫曼树
设计并输出哈夫曼编码
将文本转换成 01 编码
...
\subsubsection{1. 需求分析}
\paragraph{分析总结逻辑结构}
字符频度使用一维线性数组存储；哈夫曼树采用静态结构体数组存储，属于顺序存储二叉树；各字符哈夫曼编码存放在二维字符数组；原始文本、二进制编码串使用一维字符数组线性存储。
\paragraph{分析总结运算集合}
字符频度统计、选取权值最小两个结点、哈夫曼树构造、叶子回溯生成前缀编码、明文批量编码、二进制编码逐位译码、菜单交互控制。
\subsubsection{2. 概要设计}
\paragraph{存储结构设计}
静态哈夫曼结点结构体 HTNode：包含字符、权值、双亲下标、左孩子下标、右孩子下标；
一维数组 chars 保存去重字符，freq 数组保存对应字符出现次数；
二维字符数组 HC 存储每个叶子字符的 01 哈夫曼编码。
\paragraph{算法设计思路}
频度统计：遍历输入字符串，查重累加每个字符出现次数；
构造哈夫曼树：循环选出两个无父结点的最小权值结点，合并生成新中间结点，更新父子关系；
生成编码：从叶子结点向上回溯至根结点，左分支记 '0'、右分支记 '1'，逆序拼接得到标准编码；
文本编码：遍历原文每个字符，匹配对应编码拼接成完整 01 串；
译码：从根结点逐位读取 0/1 向下遍历树，到达叶子则输出字符并重置指针回到根，继续解析下一组编码。
\subsubsection{3. 详细设计}
\paragraph{源代码（带注释）}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{HaffmanCoding-2026-06-25-081417.png}
    \caption{哈夫曼编码示意图}
    \label{fig:Haffman-coding}
\end{figure}
...
\paragraph{调试过程遇到的问题与解决}
\begin{enumerate}
\item 未统计字符直接执行编码 / 译码功能：增加全局标记变量 hasData 拦截非法操作，提示先执行字符统计功能；
\item Select 函数选出的两个最小结点顺序颠倒：比较权值后交换 s1、s2，保证左子树权值更小；
\item 生成哈夫曼编码字符顺序颠倒：从叶子向上遍历至根结点，倒序存入临时数组，截取有效起始位置复制得到正确编码；
\item 译码识别单个字符后无法连续解析下一个字符：每次遍历到叶子结点输出字符后，将遍历指针重置回根结点；
\item 多次重复执行建树功能数据覆盖：重复调用 CreateHuffmanTree 会完整重置树数组，无逻辑错误，不影响运行结果；
\item 仅支持无空格文本输入
\end{enumerate}
\subsubsection{5. 小结}
本实验实现了哈夫曼编码完整流程，掌握贪心算法构造最优二叉树、前缀编码生成及译码方法，理解了数据压缩基本原理，提升了树结构应用能力。


\subsection {3.并查集}
\subsubsection {任务}
用树形结构来表示集合，树的每个结点代表一个集合元素。实现查找（判定）元素所 属集合的运算、实现集合的并运算。 功能要求： （1）建立并输出集合的存储结构； （2）设计并实现查找（判定）元素所属集合的运算； （3）设计并实现集合的并运算。
界面要求：程序运行后，给出菜单项的内容和输入提示： 1. 2. 3. 0. 建立集合 查找（判定）元素所属集合 集合的并运算 退出 请选择0-3：

\subsubsection{1. 需求分析}
\paragraph{分析总结逻辑结构}
采用树形逻辑结构存储集合，每个节点代表集合元素，根节点为集合代表元。
\paragraph{分析总结运算集合}
实现集合初始化构建、元素根节点查找、两个集合合并三大基础运算，搭配菜单交互使用。
\subsubsection{2. 概要设计}
\paragraph{存储结构设计}
采用一维数组存储父节点，下标为元素，数组值为父节点，实现树形静态存储。
find 函数带路径压缩优化查找，unite 函数按根合并集合，模块化设计各功能子程序。
\paragraph{算法设计思路}
\subsubsection{3. 详细设计}
\paragraph{源代码（带注释）}
\begin{figure}[H]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{FindSet-2026-06-24-032329.png}
        \caption{并查集1}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{FindSet-2026-06-24-032414.png}
        \caption{并查集2}
    \end{subfigure}
...
\subsubsection{5. 小结}
本实验实现并查集的查找、合并与路径压缩操作，可高效处理元素连通性判定，理解集合分治思想，优化查询效率。

\section{模块三}


\subsection {1.最小代价生成树}
\subsubsection {任务}
建立图并求图的最小代价生成树。 功能要求： （1）建立图的存储结构（邻接矩阵或邻接表），能够输入图的顶点、边，以及边上的权 值的信息，存储到相应存储结构中，并输出图的相关信息。 （2）利用Prim算法或Kruskal算法求图的最小代价生成树。
\subsubsection{1. 需求分析}
\paragraph{分析总结逻辑结构}
逻辑结构采用邻接矩阵存储无向带权图，顶点与边为二维线性结构。
\paragraph{分析总结运算集合}
运算包含图构建、矩阵输出、Prim 与 Kruskal 生成最小生成树，辅以边排序、并查集判连通。
\subsubsection{2. 概要设计}
\paragraph{存储结构设计}
存储用邻接矩阵存图，Edge 结构体存边，数组实现并查集。
\paragraph{算法设计思路}
Prim 借助 lowcost 数组选最短邻边；Kruskal 快排边，配合并查集规避环路生成生成树。
\subsubsection{3. 详细设计}
\paragraph{源代码（带注释）}
...
\begin{figure}[H]
    ...
    \begin{subfigure}{0.9\textwidth}
        \centering
        \includegraphics[width=\textwidth]{BinaryTree-2026-06-24-105026.png}
        \caption{图}
    \end{subfigure}
    ...
    \begin{subfigure}{0.35\textwidth}
        \centering
        \includegraphics[width=\textwidth]{MinimumSpanningTree-2026-06-24-105302.png}
        \caption{prime}
    \end{subfigure}
    \hfill
    \begin{subfigure}{0.35\textwidth}
...
\subsubsection{5. 小结}
本实验实现两种最小生成树算法，掌握邻接矩阵建图，理解 Prim 贪心找点、Kruskal 贪心选边思路，熟练运用并查集解决环路判断问题。

\subsection {2.拓扑排序及关键路径}
\subsubsection {任务}
建立图并实现图的拓扑排序和关键路径。 功能要求： （1）建立图的存储结构（邻接矩阵或邻接表），能够输入图的顶点和边以及边上的权值 等信息，存储到相应存储结构中，并输出图的结构。 （2）对图进行拓扑排序并输出结果。 （3）在（2）的基础上求出图的关键路径并输出关键路径以及源点到汇点的最长路径长度。
\subsubsection{1. 需求分析}
\paragraph{分析总结逻辑结构}
本实验采用邻接表存储带权有向图，实现图结构建立与输出。
\paragraph{分析总结运算集合}
\subsubsection{2. 概要设计}
主要运算包括拓扑排序判环、顶点发生时间计算、筛选关键活动并求解工程关键路径与总工期。
\paragraph{存储结构设计}
采用邻接表存储有向图，记录顶点入度。利用栈实现拓扑排序，正向遍历求顶点最早时间，反向遍历求最迟时间，通过时间差值筛选关键路径。
\paragraph{算法设计思路}
\subsubsection{3. 详细设计}
\paragraph{源代码（带注释）}
代码：
...
\begin{verbatim}
请输入顶点数、边数：6 8
输入第1条边 起点u 终点v 权值w：0 1 3
输入第2条边 起点u 终点v 权值w：0 2 2
输入第3条边 起点u 终点v 权值w：1 3 2
输入第4条边 起点u 终点v 权值w：1 4 3
输入第5条边 起点u 终点v 权值w：2 3 4
输入第6条边 起点u 终点v 权值w：2 5 3
输入第7条边 起点u 终点v 权值w：3 5 2
输入第8条边 起点u 终点v 权值w：4 5 1

图邻接表
顶点0：(邻接: 2, 权: 2) (邻接: 1, 权: 3) 
顶点1：(邻接: 4, 权: 3) (邻接: 3, 权: 2) 
顶点2：(邻接: 5, 权: 3) (邻接: 3, 权: 4) 
顶点3：(邻接: 5, 权: 2) 
顶点4：(邻接: 5, 权: 1) 
顶点5：
...
\subsubsection{5. 小结}
本实验实现有向图拓扑排序与关键路径算法，能够判断有向图是否存在环，熟练掌握 ve、vl 求解方法，理解 AOE 网关键活动决定工程最短工期的原理。

\subsection {3.交通咨询系统}
\subsubsection {任务}
设计一个简易交通咨询系统，能让旅客咨询从一个城市到另一个城市之间的最短路径。 功能要求： （1）用图结构表示城市之间的交通联系，图中的顶点为城市，边上的权值为两城市之间 的距离。建立交通网络图的存储结构，并输出；
（2）求一个城市到其他城市的最短路径（Dijkstra算法），输出路径顶点序列和总距离； （3）求任一对城市之间的最短路径（Floyd算法），输出路径顶点序列和总距离。
\subsubsection{1. 需求分析}
\paragraph{分析总结逻辑结构}
采用邻接矩阵存储城市交通无向图，顶点为城市、边权代表路程。
\paragraph{分析总结运算集合}
运算包含交通图构建打印、Dijkstra 单源最短路径求解、Floyd 算法求解全部顶点对最短路径并输出路径长度。（2）概要设计
\subsubsection{2. 概要设计}
\paragraph{存储结构设计}
存储使用邻接矩阵保存路网，一维数组存城市名。Dijkstra 借助 dist、visited、path 数组松弛更新；Floyd 用 distF、pathF 二维数组，三层循环枚举中转点更新最短路径。
\paragraph{算法设计思路}
\subsubsection{3. 详细设计}
\paragraph{源代码（带注释）}
代码：
...
\subsubsection{5. 小结}
本交通咨询系统实现两种经典最短路径算法，Dijkstra 适合单源查询，Floyd 可一次性算出所有城市间路径，掌握多源最松弛更新原理，仅 Floyd 路径记录逻辑存在缺陷。


\section{模块四}
\subsection {1.基于线性表的查找（学生信息管理）}
\subsubsection {任务}
建立学生信息表，实现顺序查找、折半查找。 要求： （1）输入的学生数据包含学号、姓名、总成绩等多个数据项。 （2）输出：查找成功输出找到的数据的所有数据项，查找失败也要给出相应提示信息。 （3）分析总结算法各自的特点以及ASL。
\subsubsection{1.需求分析}
\paragraph{分析总结逻辑结构}
以结构体线性表存储学生学号、姓名与成绩，提供顺序、折半两种学号查找功能，成功输出完整信息，失败给出提示。
\paragraph{分析总结运算集合}
\subsubsection{2.概要设计}
采用结构体数组作为有序线性表存储学生数据。顺序查找逐一遍历匹配学号；折半查找仅适用于有序表，通过不断缩小区间快速定位目标。
\paragraph{存储结构设计}
\paragraph{算法设计思路}
\subsubsection{3.详细设计}
\paragraph{源代码（带注释）}
...
\subsubsection{5. 小结}
本实验采用结构体建立学生信息表，以学号作为关键字，实现了顺序查找和折半查找两种查找方法，并统计了查找过程中的比较次数。通过实验发现，顺序查找的实现简单，不需要对数据进行排序，适用于无序表和各种存储结构，但随着数据规模的增加，其查找效率会明显下降。折半查找通过不断缩小查找区间来定位目标元素，平均查找长度远小于顺序查找，因此具有更高的查找效率，但必须保证查找表有序。



顺序查找成功时的平均查找长度（Average Search Length，ASL）为：
顺序查找平均查找长度：
$$
ASL = \frac{n+1}{2}
$$
...
O(\log_2 n)
$$
通过本次实验，加深了对查找表、关键字、平均查找长度（ASL）等概念的理解，掌握了顺序查找和折半查找的实现方法及适用条件，并认识到算法效率与数据组织方式之间的密切关系。在实际应用中，应根据数据是否有序以及查找效率要求选择合适的查找算法。对于无序表或数据经常发生变化的情况，顺序查找具有较好的适应性；而对于静态有序表，折半查找能够显著提高查找效率。

\subsection {2.基于树表的查找(文本词汇统计）}
\subsubsection {任务}
读取一个文本文件，统计文件中每个单词的出现次数，并按字典序输出。
要求：
（1）使用二叉搜索树存储单词及其出现次数。
（2）提供二叉搜索树上的插入、查找和中序遍历。
（3）分析并总结二叉搜索树的特点以及ASL。
\subsubsection{1. 需求分析}
\paragraph{分析总结逻辑结构}
采用二叉搜索树链式存储单词与词频，属于非线性树形结构，左子树单词字典序小于根，右子树大于根，天然具备有序特性。
\paragraph{分析总结运算集合}
包含文件字符读取、单词拆分小写转换、二叉树节点查找、插入计数、中序遍历输出、树内存释放等操作。
\subsubsection{2. 概要设计}
\paragraph{存储结构设计}
二叉链表作为存储载体，每个节点存储单词字符串、出现次数，搭配左右孩子指针；文件文本临时用一维字符数组缓存单词。
\paragraph{算法设计思路}
读取文本拆分标准化单词，递归查找 BST，存在则词频自增，不存在则新建节点插入；中序遍历按字典序输出全部单词，结束递归销毁整棵树释放内存。
\subsubsection{3. 详细设计}
\paragraph{源代码（带注释）}
代码：
\begin{verbatim}
...
\begin{verbatim}
10
12 4 19 7 3 25 10 1 16 8 
1 3 4 7 8 10 12 16 19 25 
\end{verbatim}
\subsubsection{5. 小结}
二叉搜索树的查找效率与树的形态有关。

当二叉搜索树较为平衡时，成功查找的平均查找长度（Average Search Length，ASL）约为：
...
O(n)
$$
二叉搜索树左小右大，中序遍历可实现字典序输出。其随机数据下查找插入效率高，有序数据会退化为单链表，有序场景可采用平衡树优化性能。


\subsection {3.基于哈希查找实现员工信息管理}
\subsubsection {任务}
建立员工信息管理表，实现哈希查找。 要求： （1）确定哈希函数以及解决冲突的方法。 （2）员工信息包含工号、姓名、部门等数据项，实现员工的添加、查找、删除和遍历。 （3）分析总结哈希查找的特点。
\subsubsection{1. 需求分析}
\paragraph{分析总结逻辑结构}
采用一维数组存储哈希表，属于顺序线性结构，通过哈希映射将员工工号映射至数组下标存储记录。
\paragraph{分析总结运算集合}
包含哈希映射、冲突探测、员工增删查、全表遍历、状态标记等基础操作。
\subsubsection{2. 概要设计}
\paragraph{存储结构设计}
定长数组作为哈希表，结构体存员工信息，增设状态位区分空、占用、已删三种存储单元。
\paragraph{算法设计思路}
除留余数法构造哈希函数，线性探测法处理冲突，标记删除避免查找断链，配套菜单实现交互。
\subsubsection{3. 详细设计}
\paragraph{源代码（带注释）}
代码：
\begin{verbatim}
...
请选择：4
哈希地址        工号    姓名    部门
 96             202601  LiMing  研发部
 97             202602  ZhangHua        市场部
员工信息管理系统
1. 添加员工
2. 查找员工
3. 删除员工
...
\end{verbatim}
\subsubsection{5. 小结}
本程序基于哈希表实现员工管理，哈希查找平均时间复杂度接近 O (1)，查找效率极高；线性探测易产生堆积，删除需标记不可直接置空，适合高频查询场景。

\section{模块五}


\subsection {1.各种排序实现对比}
\subsubsection {任务}
利用相关排序算法，将随机生成的一组整数（20<=个数<=100）按递增的顺序排好。 要求： （1）实现直接插入排序、希尔排序，冒泡排序、快速排序，简单选择排序、堆排序等算法。 （2）输出的形式：数字大小逐个递增的数列。 （3）比较不同排序算法的时间复杂度、空间复杂度，优缺点，以及适用场合。
\subsubsection{1. 需求分析}
\paragraph{分析总结逻辑结构}
采用一维顺序数组存储待排序整数，属于连续线性顺序存储结构，元素下标对应数字位置，便于批量遍历与移位交换。
\paragraph{分析总结运算集合}
核心操作包含元素比较、相邻 / 跨位置交换、数组移位、分组插入、堆调整、递归划分等基础运算。
\subsubsection{2. 概要设计}
\paragraph{存储结构设计}
静态一维数组存放待排序数字，下标从 1 开始适配各类排序算法边界逻辑，统一数组接口复用全部排序函数。
\paragraph{算法设计思路}
分别实现插入类、交换类、选择类共六种经典排序；统一递增排序标准，完成多算法横向对比，分析复杂度与适用场景。
\subsubsection{3. 详细设计}
\paragraph{源代码（带注释）}
代码：
\begin{verbatim}
...
\begin{itemize}
    \item 直接插入排序 $O(n^2)$ 
    \item 希尔排序 最优$O(n)$ 最差$O(n^2)$
    \item 冒泡排序 $O(n^2)$
    \item 快速排序 $O(nlogn)$ 最差$O(n^2)$
    \item 堆排序 $O(nlogn)$
\end{itemize}
